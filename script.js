...
function initMap() {
  ...
  // ── Load topojson-client dynamically (no index.html change needed) ──
  ...
  // ── Draw a GeoJSON ring onto offscreen canvas (pure equirectangular) ──
  // offscreen canvas IS equirectangular: x=(lon+180)/360*W, y=(90-lat)/180*H
  // so inverse is exact: lat=90-(y/H)*180, lon=-180+(x/W)*360
  ...
  function buildDotCache(world) {
    ...
    // Convert TopoJSON → GeoJSON land feature
    ...
    land.features.forEach(f => {
      ...
      polys.forEach(poly => {
        // Use evenodd so holes (inner rings) cut out correctly
        offCtx.save();
        offCtx.beginPath();
    ...
    // Sample — because offscreen is equirectangular, inverse is mathematically exact
    const imgData = offCtx.getImageData(0, 0, OW, OH).data;
    for (let y = 0; y < OH; y += DOT_STEP) {
      for (let x = 0; x < OW; x += DOT_STEP) {
        const i = (y * OW + x) * 4;
        if (imgData[i] > 200) {   // white pixel = land
          dotCache.push({
            lat:  90 - (y / OH) * 180,
            lon: -180 + (x / OW) * 360
          });
  ...
  loadScript('https://unpkg.com/topojson-client@3/dist/topojson-client.min.js')
    ...
    .catch(() => { mapReady = true; }); // fail gracefully — map just shows blank
